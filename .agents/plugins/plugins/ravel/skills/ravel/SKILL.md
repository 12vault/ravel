...
- Cross-project questions: use `ravel merge` for explicit `alias=graph-directory` inputs, or `ravel global add|list|query|context|build` for a reusable private local registry. Merged IDs and paths are project-namespaced; do not expose registry paths in answers.
- Pull-request impact: use `ravel prs` only when the user asks for live GitHub PR state or supplies an offline `--manifest`. Live mode delegates network access to the authenticated `gh` CLI. Use `--conflicts` for file/community overlap and `--json` for structured results.
- MCP-capable hosts may launch `ravel mcp --out <graphdir>` and use its read-only `query`, `context`, `explain`, `path`, and reverse-impact `affected` tools. Keep `context` token-bounded and treat explicit unresolved results as unresolved. The optional `--transport http` server must be user-started; keep the default loopback bind, pass secrets through `--api-key-env`, and require explicit user authorization before any non-loopback bind.
