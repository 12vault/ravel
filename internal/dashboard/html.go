...
<body>
<header><h1>Ravel</h1><input id="search" placeholder="Search nodes, paths, metadata"><select id="kind"><option value="">All kinds</option></select><span id="count" class="muted"></span></header>
...
const canvas=document.querySelector('#graph'),ctx=canvas.getContext('2d'),search=document.querySelector('#search'),kind=document.querySelector('#kind'),detail=document.querySelector('#detail'),count=document.querySelector('#count');
...
function filter(){const q=search.value.toLowerCase(),k=kind.value;visible=G.nodes.filter(n=>(!k||n.kind===k)&&(!q||JSON.stringify(n).toLowerCase().includes(q))).slice(0,1200);count.textContent=visible.length+'/'+G.nodes.length;layout()}
...
canvas.onclick=e=>{const r=canvas.getBoundingClientRect(),x=e.clientX-r.left,y=e.clientY-r.top;let best=null,d=14;for(const p of points){const n=Math.hypot(p.x-x,p.y-y);if(n<d){d=n;best=p}}if(best)show(best.n.id)};search.oninput=filter;kind.onchange=filter;addEventListener('resize',resize);filter();resize();
</script></body></html>`
